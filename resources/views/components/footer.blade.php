<footer class="bg-gray-800 text-gray-400 py-4"> {{-- Example dark gray background and light text --}}
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"> {{-- Container to center content --}}
        <div class="flex justify-between items-center"> {{-- Flexbox for layout --}}
            <div>
                <p>© {{ date('Y') }} PS-Call-Manager. All rights reserved.</p> {{-- Copyright notice --}}
            </div>
            <div>
                {{-- Optional: Footer Links (Social media, etc.) --}}
                <a href="#" class="hover:text-white mr-4">Terms of Service</a>
                <a href="#" class="hover:text-white">Privacy Policy</a>
            </div>
        </div>
    </div>
</footer>
